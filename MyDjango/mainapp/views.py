...
def cars(request):
    sales = Sale.objects.filter(sale_name='Auto')
    for i in sales:
        if i.sale_name == 'Auto':
            s = i.discount_amount
    auto = Car.objects.all()
    for el in auto:
        if el.year_of_release == 2020:
            el.sale_price = el.price * (100 - (s + 5)) / 100
        elif el.year_of_release == 2021:
            el.sale_price = el.price * (100 - s) / 100
    return render(request, 'mainapp/sale_car_list.html',
                  {'title': 'Акции', 'auto': auto, 'sales': sales})
